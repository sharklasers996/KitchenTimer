...
RTCTime::RTCTime(const uint16_t yr, const uint8_t mon, const uint8_t date,
...
void RtcModule::init(const uint8_t ce_pin, const uint8_t io_pin,
...
void RtcModule::writeOut(const uint8_t value, bool readAfter)
{
...
uint8_t RtcModule::readRegister(const uint8_t reg)
{
...
void RtcModule::writeRegister(const uint8_t reg, const uint8_t value)
{
  const SPISession s(ce_pin_, io_pin_, sclk_pin_);
...
void RtcModule::writeProtect(const bool enable)
{
  writeRegister(kWriteProtectReg, (enable << 7));
}

void RtcModule::halt(const bool enable)
{
...
RTCTime RtcModule::time()
{
  const SPISession s(ce_pin_, io_pin_, sclk_pin_);

  RTCTime t(2099, 1, 1, 0, 0, 0, RTCTime::kSunday);
  ...
  t.day = static_cast<RTCTime::Day>(bcdToDec(readIn()));
  t.yr = 2000 + bcdToDec(readIn());
  return t;
}

void RtcModule::time(const RTCTime t)
{
...
void RtcModule::writeRam(const uint8_t address, const uint8_t value)
{
  if (address >= kRamSize)
  {
    return;
...
uint8_t RtcModule::readRam(const uint8_t address)
{
  if (address >= kRamSize)
  {
    return 0;
...
void RtcModule::writeRamBulk(const uint8_t *const data, int len)
{
  if (len <= 0)
  {
    return;
...
void RtcModule::readRamBulk(uint8_t *const data, int len)
{
  if (len <= 0)
  {
    return;
